...
outdir=${dir}/sly_acr_dis_fasta
for inp in ${dir}/sly_acr_dis/*.txt
do
...
do
    cluster=C${i}
    for inp in ${dir}/sly_acr_dis/${cluster}/*.txt
    do
        python FastaManager_modified.py \
            -f getseq2 \
            -fasta ${dir}/peak.coord.fa \
