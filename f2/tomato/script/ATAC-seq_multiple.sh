...
sample1=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $4}' $config)
sample2=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $5}' $config)
echo ${sample}
echo ${sample1}
echo ${sample2}
print_separator
...
echo "Please finish replicates split version first!"
...
samtools merge ${outdir6}/${sample}.sorted.bam ${outdir}/${sample1}.sorted.bam ${outdir}/${sample2}.sorted.bam
# samtools index bam 
...
macs2 callpeak -t ${outdir3}/${sample}.flt.bam \
    ...
    --outdir ${outdir7}/${sample}_merge_peakcall
print_separator 
###########################################
...
outdirx=${path}/merge_narrowpeak_copy
mkdir ${outdirx}

for file in ${outdir7}/${sample}_merge_peakcall/*.narrowPeak
do
    cp ${file} ${outdirx}
done
###########################################