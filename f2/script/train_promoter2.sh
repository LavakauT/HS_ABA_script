#!/bin/bash
#SBATCH -J ML
#SBATCH -o ML_acr_promoter_%a.out
#SBATCH --ntasks 1
...
do 
    if [ -s "${inp}.top_feat_lasso" ]; then
        ...
        python ML_classification_modified.py \
            -df ${inp} -test ${inp}.test -feat ${inp}.top_feat_lasso -cl_train pos,neg -alg SVM -apply all -n 10 -cv_num 10 -gs_n 10 -n_jobs 30 -plots T
    else
        echo "Processing "${inp}" in LogReg without lasso features"
        python ML_classification_modified.py \
            -df ${inp}  -test ${inp}.test -cl_train pos,neg -alg SVM -apply all -n 10 -cv_num 10 -gs_n 10 -n_jobs 30 -plots T
    fi
done
...
do 
    if [ -s "${inp}.top_feat_lasso" ]; then
        echo "Processing "${inp}" in LogReg with lasso features"
        python ML_classification_modified.py \
            -df ${inp} -test ${inp}.test -feat ${inp}.top_feat_lasso -cl_train pos,neg -alg LogReg -apply all -n 10 -cv_num 10 -gs_n 10 -n_jobs 30 -plots T
    else
        echo "Processing "${inp}" in LogReg without lasso features"
        python ML_classification_modified.py \
            -df ${inp}  -test ${inp}.test -cl_train pos,neg -alg LogReg -apply all -n 10 -cv_num 10 -gs_n 10 -n_jobs 30 -plots T
    fi
done
...
do 
    if [ -s "${inp}.top_feat_lasso" ]; then
        echo "Processing "${inp}" in LogReg with lasso features"
        python ML_classification_modified.py \
            -df ${inp} -test ${inp}.test -feat ${inp}.top_feat_lasso -cl_train pos,neg -alg RF -apply all -n 10 -cv_num 10 -gs_n 10 -n_jobs 30 -plots T
    else
        echo "Processing "${inp}" in LogReg without lasso features"
        python ML_classification_modified.py \
            -df ${inp}  -test ${inp}.test -cl_train pos,neg -alg RF -apply all -n 10 -cv_num 10 -gs_n 10 -n_jobs 30 -plots T
    fi
done
