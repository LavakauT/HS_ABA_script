...
sub.dir2<- list.dirs(dir2,
                     full.names = F,
                     recursive = F)
# tfbm database from MEME-Suit (Arabidopsis)
tfbm <- read_meme('/RAID1/working/R425/lavakau/kmer/Athaliana_TFBM_v1.01.tm.index.direct.index_modified_meme.txt')
...
for (y in seq_along(sub.dir)) {
  ...
  comparisons <- compare_motifs(c(df, tfbm),
  ...
  # tfbm motif family strings manipulation
  motif.family <- df3
  ...
  fam <- data.frame(t(table(output$motif))) %>% 
    pull(Var2) %>% 
    as.character()
  ...
  for (x in seq_along(fam)) {
    if(x == 1){
      output %>% 
        filter(motif %in% fam[x]) %>% 
        mutate(new_name2 = paste(cluster, motif, 1:nrow(.), sep = '_')) -> out
    }else{
      output %>% 
        filter(motif %in% fam[x]) %>% 
        mutate(new_name2 = paste(cluster, motif, 1:nrow(.), sep = '_')) -> out2
      rbind(out, out2) -> out
    }
  }
  out %>% 
  ...
  alt <- read.delim(paste(dir2, sub.dir2[y], paste0(sub.dir2[y], '_top_sim_tfbm.txt'), sep = '/')) %>% 
    pull(kmers)
  ...
  write.table(output2,
              paste(dir3, paste0(file_name, '_top5_consensus_tfbm.txt'), sep = '/'),
              row.names = F,
              quote = F,
              sep = '\t') # export top5
  write.table(out %>% 
                rownames_to_column(., var = 'name'),
              paste(dir3, paste0(file_name, '_all_consensus_tfbm.txt'), sep = '/'),
              row.names = F,
              quote = F,
              sep = '\t') # export all
  
  save.image(file = paste(dir3, paste0(file_name, '_consensus_tfbm.RData'), sep = '/'))
