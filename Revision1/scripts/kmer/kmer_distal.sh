#!/bin/bash
#SBATCH -J KMER
#SBATCH -o KMER_FET_distal_%a.out
#SBATCH --ntasks 1
...
# Convert all top-level distal .txt to .fa
for inp in "${dir}"/peak_distal/*.txt; do
    [[ -e "$inp" ]] || continue
...
# Convert per-cluster distal .txt to .fa (clusters 1..4)
for i in {1..4}; do
    cluster="cluster_${i}"
    for inp in "${dir}/peak_distal/${cluster}"/*.txt; do
        [[ -e "$inp" ]] || continue
        python FastaManager_modified2.py \
            -f getseq2 \
            -fasta "${dir}/peak.coord.fa" \
...
echo "Running: ${sample}"
indir="${dir}/peak_distal"
outdir="${dir}/peak_distal_fasta"
