...
outdir3=${indir2}/BINDetect_pd_one_condition2

mkdir ${outdir3}

# predict binding for a single condition 
...
do
    if [[ "$a_full" == *-HS-merge_footprints.bw ]]; then
        ...
        echo "Processing: $a_full under heat stress"
        TOBIAS BINDetect --motifs /RAID1/working/R425/lavakau/kmer/dap/hsfa_1000/meme.txt_0.01_DAP-seq \
            --signals ${a_full} \
            --genome MpTak_v6.1r2.genome.fasta \
            --peaks merged_available.bed \
            --peak_header merged_header.txt \
            --outdir ${outdir3}/${a_g}_HS \
            --cond_names HS \
            --cores 20
        echo "Finished processing : $a_full"
    fi
done
