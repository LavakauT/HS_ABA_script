...
outdir3=${indir2}/BINDetect_pd_DAP
outdir4=${indir2}/BINDetect_pd_DAP/promoter3
outdir5=${indir2}/BINDetect_pd_DAP/distal3
        ...
        do
            if [[ "$b_full" == *-CK-merge.footprint.bw ]]; then
                ...
                if [[ "$a_g" == "$b_g" ]]; then
                    ...
                    TOBIAS BINDetect --motifs /RAID1/working/R425/lavakau/kmer/dap/hsfa_600/meme.txt_5E-02_DAP \
                        --signals ${a_full} ${b_full} \
                        --genome MpTak_v6.1r2.genome.fasta \
                        --peaks DE_peaks_hclust_available2_promoter.bed \
            ...
            if [[ "$b_full" == *-CK-merge.footprint.bw ]]; then
                ...
                if [[ "$a_g" == "$b_g" ]]; then
                    ...
                    TOBIAS BINDetect --motifs /RAID1/working/R425/lavakau/kmer/dap/hsfa_600/meme.txt_5E-02_DAP  \
                        --signals ${a_full} ${b_full} \
                        --genome MpTak_v6.1r2.genome.fasta \
                        --peaks DE_peaks_hclust_available2_distal.bed \
                        --peak_header DE_peaks_hclust_header.txt \
