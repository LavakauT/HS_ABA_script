...
# p_peak <- 'f1/narrowpeak/merge'
# p_plot <- 'f1/plots'
# p_path <- 'f1/summary'
# range <- data.frame(group1 = paste(c('Tak1', 'hsfa', 'hsfb', 'dko'), 'CK', sep = '-'),
#                     group2 = paste(c('Tak1', 'hsfa', 'hsfb', 'dko'), 'HS', sep = '-'))
# 
# for (i in 1:nrow(range)) {
#   file_name1 <- paste0(range[i,]$group1, '.peaks.narrowPeak')
#   file_name2 <- paste0(range[i,]$group2, '.peaks.narrowPeak')
#   
...
#                   color = group)) + 
#     geom_density(linewidth = 1, key_glyph = draw_key_point) +
